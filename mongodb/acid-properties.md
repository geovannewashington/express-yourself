...
- Atomicity: Transactions are treated as single units; either all operations within a transactions succeed, or nonde do.
- Consistency: Ensures that a transaction maintains data integrity and moves the database from one valid state to another.
- Isolation: Guarantees that concurrent transactions do not interfere with each other, makingit seem
  as if they are processed sequentially.
- Durability: Ensures that once a transaction is committed, its change are permanent and will persist
